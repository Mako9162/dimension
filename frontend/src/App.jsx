...
import PublicView from './PublicQuote';
import Registry from './Registry';
import Login from './Login';
import { WORKSHOP_LOGO, WORKSHOP_LOGO_DARK } from './brand';
...
function Admin() {
  ...
  useEffect(() => {
    if (!logged || creating || busy) return;
    const controller = new AbortController();
    ...
    const update = async () => {
      ...
      try {
        if (selected?.id) {
          const latest = await api(`/quotes/${selected.id}`, { signal: controller.signal });
          setSelected(latest);
          setSharePath(latest.publicToken ? `${location.origin}/q/${latest.publicToken}` : '');
        } else setRevision(v => v + 1);
      } catch { /* Conserva la última vista; los errores de sesión se gestionan en api. */ }
  ...
  }, [logged, creating, busy, selected?.id]);
  ...
  async function refreshQuote() { const latest = await api(`/quotes/${selected.id}`); setSelected(latest); setSharePath(latest.publicToken ? `${location.origin}/q/${latest.publicToken}` : ''); setRevision(v => v + 1); }
