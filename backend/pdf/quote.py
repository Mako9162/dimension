...
WIDTH = 499.27
LOGO = Path(__file__).resolve().parents[2] / 'frontend/public/brand/taller-dimension.png'
...
def get_canvas_image(img_str):
    ...
    try:
        ...
        if img_str == '/brand/taller-dimension.png' and LOGO.exists():
...
def resolve_image(img_str, max_w=140, max_h=50):
    ...
    try:
        ...
        if img_str.startswith('data:image'):
            header, encoded = img_str.split(',', 1)
            img_data = base64.b64decode(encoded)
            return Image(io.BytesIO(img_data), width=max_w, height=max_h)
        if img_str == '/brand/taller-dimension.png' and LOGO.exists():
            return Image(str(LOGO), width=max_w, height=max_h)
        if img_str.startswith('/'):
            local_path = Path(__file__).resolve().parents[2] / 'frontend/public' / img_str.lstrip('/')
            if local_path.exists():
                return Image(str(local_path), width=max_w, height=max_h)
        if Path(img_str).exists():
            return Image(img_str, width=max_w, height=max_h)
    except Exception:
        pass
    return None
